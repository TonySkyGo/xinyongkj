 ...
 */
$(function() {
    $("div#wallet-tab a.center-state-btn").click(function() {
       $("div#wallet-tab a.center-state-btn").removeClass("color-hover");
       $(this).addClass("color-hover");
       var i=$(this).index();
       if( $(this).index()==i){
           $("#balance-table table").hide();
           $("#balance-table table").eq(i).show();
       }
    })
})

//输入密码框效果代码
var box = document.getElementsByClassName("zhifumima-possword-id1")[0];
...
function func() {
    for (var i = 0; i < pawDivCount; i++) {
        ...
        paw[i].onkeyup = function (event) {
            ...
            if ((event.keyCode==46)|| (event.keyCode>=48 && event.keyCode<=57)||(event.keyCode>=96 && event.keyCode<=105)||(event.keyCode>=37 && event.keyCode<=40)||event.keyCode==110||event.keyCode==190)  {    /*输入0-9*/
                changeDiv();
